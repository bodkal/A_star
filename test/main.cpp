...
void bild_board(short** board,vector<pair<short,short>> obs);
...
int main()
{
    ...
    for (short i = 0; i < x ; ++i) {
        short num = rand() % (x-1) + 1;
        short xxx = rand() % (x-1) + 1;
        short yyy = rand() % (x-1) + 1;

        for (short i = num; i < num + rand() % (y - num)+1; ++i) {
            obs.push_back({xxx, i});
        }
        num = rand() %(x-1) + 1;
        for (short i = num; i < num + rand() % (x - num) +1; ++i) {
            obs.push_back({i, yyy});
        }
    }
    ...
    vector<short> x_o;
    vector<short> y_o;
    for (short i = 0; i < obs.size(); ++i) {
        x_o.push_back(obs[i].first);
        y_o.push_back(obs[i].second);
    }


    bild_board(board,obs);
    vector<Int2> path2;
    ...
    xx.push_back(i.pos[0]);
    yy.push_back(i.pos[1]);
    }
...
vector<Int2> serce(short** board, short start[2], short end[2]) {
    ...
    while (open_list.size()) {
        ...
        short insert = 0;
        for (short i = 0; i < 8; i++) {
            tmp_pos[0] = open_list[0]->pos[0] + move[i][0];
            tmp_pos[1] = open_list[0]->pos[1] + move[i][1];
            if (tmp_pos[0] == end[0] && tmp_pos[1] == end[1]) {
              ...
              //  if(board[tmp_pos[0]][tmp_pos[1]]>(open_list[0].g + (abs(tmp_pos[0] - per->pos[0]) + abs(tmp_pos[1] -  per->pos[1]))))
                continue;
            }
            else {
                cell* per = new cell;
                per = open_list[0];
                float g = (per->g + (abs(tmp_pos[0] - per->pos[0]) + abs(tmp_pos[1] -  per->pos[1]))); //sqrtf(abs(per->move[0].pos[0]) + abs(per->move[0].pos[1]))));
...
void bild_board(short** board,vector<pair<short,short>> obs) {
    ...
    for(auto const &i:obs){
        board[i.first][i.second]=500;
    }
